...
1. **Scope.** Resolve the binding. Build a manifest of harness files (respect `outOfScope`). Use context-mode `ctx_batch_execute` for the listing/globs so raw config stays off-prompt. Never silently truncate — if the surface is large, say what you're sampling.
...
4. **Run the auditor.** Spawn `harness-auditor` (matching `subagent_type`) with: the target path, the file manifest, the always-loaded-vs-lazy split, and the wiring map. Discovery-only. For a large harness, fan out in parallel by lens (`dedup` / `guardrails` / `context` / `hygiene` / `feedback`) and tell the user you did.
