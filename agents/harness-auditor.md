...
- **Validate every concrete reference against the real repo.** Enumerate the specifics the harness names — path globs, app / `nx`-project names, branch names, script / migration / file names, test targets — and confirm each still resolves (filesystem, `nx show projects`, `git branch -r`). A reference that no longer resolves is dead config (a glob matching nothing, a default base branch that was deleted, a renamed or extracted app); a reference that **does** resolve is not a finding, however stale it looks. Use the ground-truth inventory the orchestrator hands you, or gather it yourself if running standalone. This is the pass that catches extraction / rename residue across skills, docs, and hooks in **one** sweep instead of needing a second.
